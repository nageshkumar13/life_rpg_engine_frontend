...
export function ChunkRow({ chunk, onToggle, onEdit, onDelete }: ChunkRowProps) {
  return (
    <div className="flex items-center gap-4 rounded-2xl border border-white/5 bg-white/[0.03] px-4 py-3">
      <AnimatedCheckbox checked={chunk.status === "DONE"} onChange={onToggle} />
      ...
      <div className="flex items-center gap-2">
        <button
          type="button"
          onClick={onEdit}
          className="rounded-xl border border-white/10 p-2 text-text-secondary transition hover:bg-elevated hover:text-text-primary"
        >
        ...
        <button
          type="button"
          onClick={onDelete}
          className="rounded-xl border border-danger/20 p-2 text-danger transition hover:bg-danger/10"
        >
